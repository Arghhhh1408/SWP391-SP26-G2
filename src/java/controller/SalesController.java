    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if ("dashboard".equals(tab)) {
                ...
                // Tra cứu bảo hành theo đơn/phiếu bán (StockOutID) + SKU.
                WarrantyLookupDAO wlDao = new WarrantyLookupDAO();
                String q = safeTrim(request.getParameter("q"));
                request.setAttribute("q", q);

                List<WarrantyLookupResult> warrantyResults = new ArrayList<>();
                Integer stockOutId = tryParseInt(q);
                if (stockOutId != null) {
                ...
                // Tra cứu trả hàng theo StockOutID và cho tạo yêu cầu từ kết quả
                ReturnLookupDAO rlDao = new ReturnLookupDAO();
                String rq = safeTrim(request.getParameter("rq"));
                request.setAttribute("rq", rq);

                List<ReturnLookupResult> returnLookupResults = new ArrayList<>();
                Integer stockOutId = tryParseInt(rq);
                if (stockOutId != null) {
                    returnLookupResults = rlDao.lookupByStockOutId(stockOutId);
                    request.setAttribute("stockOutId", stockOutId);
                } else if (rq != null && !rq.isBlank()) {
                    request.setAttribute("error", "Mã phiếu xuất (StockOutID) phải là số.");
                }
                request.setAttribute("returnLookupResults", returnLookupResults);
