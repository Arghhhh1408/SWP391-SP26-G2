...
public class OrderDetailController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (idRaw == null) {
            response.sendRedirect("sales_dashboard?tab=orders");
            return;
        }

        try {
            int stockOutId = Integer.parseInt(idRaw);
            OrderHistoryDAO dao = new OrderHistoryDAO();

            // 2. Lấy thông tin chung (Header) và danh sách sản phẩm (Items)
            // Hãy đảm bảo các hàm này trong DAO đã được sửa tên cột Name/Phone như bước trước
