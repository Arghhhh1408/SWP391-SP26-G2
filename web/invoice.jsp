...
<html>
    ...
    <body>
        <div class="receipt">
            ...
            <div class="meta">
                <div class="kv">
                    ...
                    <div class="v"><fmt:formatDate value="${now}" pattern="dd/MM/yyyy HH:mm:ss"/></div>
                </div>
                <div class="kv">
            ...
            <div class="items">
                <c:forEach items="${lastOrder.values()}" var="it">
                    <div class="item">
                        ...
                        <div class="row muted">
                            <span>${it.qty} x <span class="money"><fmt:formatNumber value="${it.price}" type="number" groupingUsed="true"/></span></span>
                            <span class="money"><fmt:formatNumber value="${it.lineTotal}" type="number" groupingUsed="true"/></span>
                        </div>
                    </div>
                </c:forEach>
            </div>
            ...
            <div class="total">
                <span>TỔNG TIỀN</span>
                <span class="money"><fmt:formatNumber value="${lastTotal}" type="number" groupingUsed="true"/> đ</span>
            </div>
        ...
        <div class="no-print">
            ...
            <button class="link-btn" type="button" onclick="backToPos()">Đóng</button>
        </div>

        <form id="finishForm" action="${pageContext.request.contextPath}/invoice/finish" method="post" style="display:none;">
        ...
        <script>
            function submitFinish() {
                ...
                // Dùng Fetch API để gửi form ngầm (AJAX)
                ...
                fetch(f.action, {
                    ...
                    body: new URLSearchParams(formData)
                        ...
                        .then(() => {
                            // Sau khi lưu đơn thành công, bảo trang cha (trang POS) reload lại
                            if (window.parent) {
                                window.parent.location.href = '${pageContext.request.contextPath}/pos';
                            }
                        })
                        .catch(err => {
            ...
            function backToPos() {
                // Khi ở trong Modal, window.parent là trang POS chính
                if (window.parent && typeof window.parent.closeInvoice === "function") {
