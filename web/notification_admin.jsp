                        ...
                        <c:choose>
                            <c:when test="${not empty notifications}">
                                <c:forEach var="n" items="${notifications}">
                                    <div style="padding: 25px; border-bottom: 1px solid var(--border-color); ${not n.read ? 'background:#fbfbff; border-left:4px solid var(--primary);' : ''}">
                                                ...
                                                <div id="notif-action-${n.notificationId}" style="display: flex; gap: 10px;">
                                                    <c:choose>
                                                        <c:when test="${n.type == 'PASSWORD_RESET_REQUEST'}">
                                                            <button class="btn btn-primary" style="background:#ffad73; padding: 6px 15px; font-size: 12px;"
                                                        ...
                                                        <c:otherwise>
                                                            <button class="btn btn-outline" style="padding: 6px 15px; font-size: 12px;"
                                                                onclick="markAsRead(${n.notificationId})">Đánh dấu đã đọc</button>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </div>
                                            </c:if>
